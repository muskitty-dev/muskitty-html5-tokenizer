...
// Entity names preserve the trailing `;` exactly as in the spec:
//   - non-legacy entities are stored as `Name;` (semicolon required)
//   - legacy entities are stored BOTH as `Name` and `Name;`
// 2231 entries, sorted alphabetically for binary search.

/// Sorted array of (entity_name, resolved_characters).
/// Entity name does NOT include leading `&` but DOES keep a
/// trailing `;` when the spec requires one (i.e. non-legacy).
